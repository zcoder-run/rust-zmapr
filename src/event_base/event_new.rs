...
pub(crate) fn new_mpsc_bounded<T>(channel: &'static str, capacity: usize) -> EventBaseResult<(MpscTx<T>, MpscRx<T>)> {
	if capacity == 0 {
	...
	Ok((MpscTx::new(channel, sender), MpscRx::new(channel, receiver)))
}

pub(crate) fn new_mpsc_bounded_default<T>(channel: &'static str) -> EventBaseResult<(MpscTx<T>, MpscRx<T>)> {
...
pub(crate) fn new_once<T>(channel: &'static str) -> (OnceTx<T>, OnceRx<T>) {
	...
	(OnceTx::new(channel, sender), OnceRx::new(channel, receiver))
}
